...
@section('content')
    <div class="row">
        ...
        <div class="col-lg-12">
            <div class="mb-3">
                <form action="{{ route('admin-kegiatanharian.update', $kegiatans->kegiatan_id ?? '') }}"
                      method="POST">
                    @csrf
                    <div class="card">
                        <div class="card-body">
                                ...
                                <div class="col-lg-6">
                                    <div class="mb-3">
                                        ...
                                        <select name="status"
                                            ...
                                            <option value="4"
                                                    {{ old('status', $kegiatans->status) == '4' ? 'selected' : '' }}>Disetujui Direktur</option>
                                            <option value="3"
                                                    {{ old('status', $kegiatans->status) == '3' ? 'selected' : '' }}>Disetujui Oleh SDM</option>
                                            <option value="2"
                                                    {{ old('status', $kegiatans->status) == '2' ? 'selected' : '' }}>Disetujui Oleh Atasan</option>
                                            <option value="1"
                                                    {{ old('status', $kegiatans->status) == '1' ? 'selected' : '' }}>Proses Pengajuan</option>
                                            <option value="0"
                                                    {{ old('status', $kegiatans->status) == '0' ? 'selected' : '' }}>Ditolak</option>
                                        </select>
                                        @error('status')
                                            <div class="invalid-feedback">
                                                {{ $message }}
                        ...
                        <div class="card-footer">
                            ...
                            <a href="{{ route('admin-kegiatanharian.index') }}"
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#selectedAtasan').select2({
                ...
                ajax: {
                    url: '{{ route('admin-kegiatanharian.getUser') }}',
                    dataType: 'json',
                    delay: 250,
            ...
            var userId = "{{ $kegiatans->user_id ?? '' }}";
            var userText = "{{ $kegiatans->nama_karyawan ?? '-' }}";
            if (userId) {
