    ...
    <script>
        $(document).ready(function() {
            ...
            let myTable = $("#myTable").DataTable({
                ...
                ajax: {
                    url: "{{ route('operator-lembur.index') }}",
                    data: function(data) {
                        ...
                        $('#generatepdf').attr('href',
                            `{{ route('operator-lembur.generatepdf') }}?status=${status}&start_date=${startDate}&end_date=${endDate}`
                        );

                        $('#generateexcel').attr('href',
                            `{{ route('operator-lembur.generateexcel') }}?status=${status}&start_date=${startDate}&end_date=${endDate}`
                        );
                    },
                },
                columns: [{
            ...
            $("#myTable").on("click", ".btn-diterima", function() {
                ...
                $.ajax({
                    url: "/operator-lembur/diterima/" + resultid, // route('admin-users.destroy', id)
            ...
            $("#myTable").on("click", ".btn-ditolak", function() {
                ...
                $.ajax({
                    url: "/operator-lembur/ditolak/" + resultid, // route('admin-users.destroy', id)
                    type: "POST", // Laravel destroy biasanya pakai method DELETE
                    data: {
                        _token: "{{ csrf_token() }}", // wajib untuk keamanan Laravel
                    },
