@extends('dashboard.layout.master')
@section('title', 'Data Izin Terlambat | RSKM Regina Eye Center')
@section('menuManajemenKehadiran', 'active')
@section('menuDataIzinTerlambat', 'active')

@section('content')
    <div class="row">
        <div class="col-lg-6">
            <h3>Data Izin Terlambat</h3>
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb"
                    style="margin-left: -15px">
                    <li class="breadcrumb-item"><a href="/dashboard"
                           class="text-primary">Dashboard</a></li>
                    <li class="breadcrumb-item active"
                        aria-current="page">Data Izin Terlambat</li>
                </ol>
            </nav>
        </div>
    </div>

    <div class="row">
        <div class="col-lg-12">
            <div class="mb-3">
                <div class="card card-side-left">
                    <div class="card-body">
                        <h4 class="card-title">Pencarian Data Izin Terlambat</h4>
                        <div class="row">
                            <div class="col-lg-3">
                                <div class="mb-3">
                                    <input type="text"
                                           class="form-control"
                                           id="searchByDate">
                                </div>
                            </div>
                            <div class="col-lg-3">
                                <div class="mb-3">
                                    <select name="status"
                                            class="form-control"
                                            id="selectedStatus">
                                        <option value=""
                                                selected>== Pilih Status Izin Terlambat ==</option>
                                        <option value="4">Disetujui Direktur</option>
                                        <option value="3">Disetujui Oleh SDM</option>
                                        <option value="2">Disetujui Oleh Atasan</option>
                                        <option value="1">Proses Pengajuan</option>
                                        <option value="0">Ditolak</option>
                                    </select>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="col-lg-12">
            <div class="mb-4">
                <div class="card">
                    <div class="card-header">
                        <a href="{{ route('karyawan-izinterlambat.create') }}"
                           class="btn btn-primary">
                            <i class="fas fa-plus"></i>
                            Tambah Data
                        </a>
                    </div>
                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="table table-bordered table-striped"
                                   id="myTable"
                                   style="width: 100%">
                                <thead>
                                    <tr>
                                        <th width="4%"
                                            style="text-align: center">#</th>
                                        <th>Nama</th>
                                        <th>Tanggal</th>
                                        <th>Jam Datang</th>
                                        <th>Alasan</th>
                                        <th>Status</th>
                                        <th>Atasan</th>
                                        <th>Aksi</th>
                                    </tr>
                                </thead>
                            </table>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="col-lg-12 mb-4">
            <h5>Status Izin Terlambat</h5>
            <div class="row"
                 id="statusSummary">
                <!-- Batal -->
                <div class="col-md-2 col-sm-4 col-6 mb-3">
                    <div class="card text-white bg-danger shadow-sm">
                        <div class="card-body p-2 text-center">
                            <h6 class="card-title mb-1">Batal</h6>
                            <span id="countBatal"
                                  class="h5 fw-bold">0</span>
                        </div>
                    </div>
                </div>

                <!-- Proses Pengajuan -->
                <div class="col-md-2 col-sm-4 col-6 mb-3">
                    <div class="card text-white bg-warning shadow-sm">
                        <div class="card-body p-2 text-center">
                            <h6 class="card-title mb-1">Proses Pengajuan</h6>
                            <span id="countProses"
                                  class="h5 fw-bold">0</span>
                        </div>
                    </div>
                </div>

                <!-- Disetujui Atasan -->
                <div class="col-md-2 col-sm-4 col-6 mb-3">
                    <div class="card text-white bg-info shadow-sm">
                        <div class="card-body p-2 text-center">
                            <h6 class="card-title mb-1">Disetujui Atasan</h6>
                            <span id="countAtasan"
                                  class="h5 fw-bold">0</span>
                        </div>
                    </div>
                </div>

                <!-- Disetujui SDM -->
                <div class="col-md-2 col-sm-4 col-6 mb-3">
                    <div class="card text-white bg-primary shadow-sm">
                        <div class="card-body p-2 text-center">
                            <h6 class="card-title mb-1">Disetujui SDM</h6>
                            <span id="countSDM"
                                  class="h5 fw-bold">0</span>
                        </div>
                    </div>
                </div>

                <!-- Disetujui Direktur -->
                <div class="col-md-2 col-sm-4 col-6 mb-3">
                    <div class="card text-white bg-success shadow-sm">
                        <div class="card-body p-2 text-center">
                            <h6 class="card-title mb-1">Disetujui Direktur</h6>
                            <span id="countDirektur"
                                  class="h5 fw-bold">0</span>
                        </div>
                    </div>
                </div>

                <!-- Total Izin Terlambat -->
                <div class="col-md-2 col-sm-4 col-6 mb-3">
                    <div class="card text-white bg-secondary shadow-sm">
                        <div class="card-body p-2 text-center">
                            <h6 class="card-title mb-1">Total Izin Terlambat</h6>
                            <span id="countTotalTerlambat"
                                  class="h5 fw-bold">0</span>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
@endsection
@push('custom-script')
    <script>
        $(document).ready(function() {
            $.ajaxSetup({
                headers: {
                    "X-CSRF-TOKEN": $('meta[name="csrf-token"]').attr("content"),
                },
            });

            {{--  Status  --}}
            $('#selectedStatus').select2({
                theme: 'bootstrap4',
            });

            var startOfMonth = moment().startOf('month');
            var endOfMonth = moment().endOf('month');

            $('#searchByDate').daterangepicker({
                startDate: startOfMonth,
                endDate: endOfMonth,
                locale: {
                    format: 'YYYY-MM-DD',
                    applyLabel: 'Terapkan',
                    cancelLabel: 'Batal',
                    customRangeLabel: 'Pilih Rentang Tanggal',
                    daysOfWeek: [
                        'Min', 'Sen', 'Sel', 'Rab', 'Kam', 'Jum', 'Sab'
                    ],
                    monthNames: [
                        'Januari', 'Februari', 'Maret', 'April', 'Mei', 'Juni',
                        'Juli', 'Agustus', 'September', 'Oktober', 'November', 'Desember'
                    ],
                    firstDay: 1
                },
                ranges: {
                    'Hari Ini': [moment(), moment()],
                    'Kemarin': [moment().subtract(1, 'days'), moment().subtract(1, 'days')],
                    '7 Hari Terakhir': [moment().subtract(6, 'days'), moment()],
                    '30 Hari Terakhir': [moment().subtract(29, 'days'), moment()],
                    'Bulan Ini': [moment().startOf('month'), moment().endOf('month')],
                    'Bulan Lalu': [moment().subtract(1, 'month').startOf('month'), moment().subtract(1, 'month').endOf('month')]
                }
            }, function(start_date, end_date) {
                myTable.draw();
            });

            // Tampilkan Data
            let myTable = $("#myTable").DataTable({
                processing: true,
                serverSide: true,
                paging: true,
                pageLength: 10,
                lengthMenu: [
                    [10, 25, 50, 100, 250],
                    [10, 25, 50, 100, 250],
                ],
                language: {
                    paginate: {
                        previous: "Sebelumnya",
                        next: "Selanjutnya",
                    },
                },
                ajax: {
                    url: "{{ route('karyawan-izinterlambat.index') }}",
                    data: function(data) {
                        data.page = Math.ceil(data.start / data.length) + 1;
                        data.search = $("#myTable_filter input").val();
                        var startDate = $('#searchByDate').data('daterangepicker').startDate.format(
                            'YYYY-MM-DD');
                        var endDate = $('#searchByDate').data('daterangepicker').endDate.format(
                            'YYYY-MM-DD');

                        data.start_date = startDate;
                        data.end_date = endDate;

                        data.status = $('#selectedStatus').val();
                    },
                    dataSrc: function(json) {
                        // Update status summary
                        $('#countBatal').text(json.statusCounts['0'] ?? 0);
                        $('#countProses').text(json.statusCounts['1'] ?? 0);
                        $('#countAtasan').text(json.statusCounts['2'] ?? 0);
                        $('#countSDM').text(json.statusCounts['3'] ?? 0);
                        $('#countDirektur').text(json.statusCounts['4'] ?? 0);

                        return json.data;
                    }
                },
                columns: [{
                        data: null,
                        render: function(data, type, row, meta) {
                            return meta.row + meta.settings._iDisplayStart + 1;
                        },
                        orderable: false,
                    },
                    {
                        data: "nama_karyawan",
                        name: "nama_karyawan",
                        defaultContent: "-",
                    },
                    {
                        data: 'tgl_izin',
                        name: 'tgl_izin',
                        defaultContent: '-',
                    },
                    {
                        data: 'jam_datang',
                        name: 'jam_datang',
                        defaultContent: '-',
                    },
                    {
                        data: 'alasan',
                        name: 'alasan',
                        defaultContent: '-',
                    },
                    {
                        data: 'status',
                        defaultContent: '-',
                        render: function(data, type, row) {
                            var status = row.status;
                            if (status == '4') {
                                return '<span class="badge badge-success">Disetujui Direktur</span>';
                            } else if (status == '1') {
                                return '<span class="badge badge-warning">Proses Pengajuan</span>';
                            } else if (status == '2') {
                                return '<span class="badge badge-info">Disetujui Oleh Atasan</span>';
                            } else if (status == '3') {
                                return '<span class="badge badge-primary">Disetujui Oleh SDM</span>';
                            } else if (status == '0') {
                                return '<span class="badge badge-danger">Ditolak</span>';
                            } else {
                                return '<span class="badge badge-secondary">Tidak Diketahui</span>';
                            }
                        }
                    },
                    {
                        data: 'nama_atasan',
                        name: 'nama_atasan',
                        defaultContent: '-',
                    },
                    {
                        data: "aksi",
                        name: "aksi",
                        orderable: false,
                        searchable: false,
                        defaultContent: "-",
                    },
                ],

                order: [
                    [1, "desc"]
                ],
            });

            $('#selectedStatus').on('change', function() {
                myTable.ajax.reload();
            });

            // Ambil data lembur dari server dan hitung total
            myTable.on('xhr.dt', function(e, settings, json, xhr) {
                let totalLembur = 0;

                if (json && json.data) {
                    json.data.forEach(function(row) {
                        totalLembur += parseFloat(row.total) || 0; // pastikan nilai numeric
                    });
                }

                // Update card Total Lembur
               $('#countTotalTerlambat').text(json.totalIzins);
            });

            // === Event Listener untuk Tombol Hapus User ===
            $("#myTable").on("click", ".btn-delete", function() {
                const resultid = $(this).data("resultid");

                if (!confirm("Apakah Anda yakin ingin menghapus data ini?")) {
                    return;
                }

                $.ajax({
                    url: "/karyawan-izin/terlambat/destroy/" + resultid, // route('admin-users.destroy', id)
                    type: "POST", // Laravel destroy biasanya pakai method DELETE
                    data: {
                        _token: "{{ csrf_token() }}", // wajib untuk keamanan Laravel
                    },
                    success: function(res) {
                        if (res.status === "success") {
                            toastr.success(res.message || "Data berhasil dihapus!");
                            myTable.ajax.reload(null, false); // reload tabel tanpa reset pagination
                        } else {
                            toastr.warning(res.message || "Gagal menghapus data.");
                        }
                    },
                    error: function(xhr) {
                        toastr.error("Terjadi kesalahan: " + xhr.responseText);
                    },
                });
            });

        });
    </script>
    <script>
        $(document).ready(function() {
            @if (Session::has('success'))
                toastr.success("{{ Session::get('success') }}");
            @endif

            @if (Session::has('error'))
                toastr.error("{{ Session::get('error') }}");
            @endif
        });
    </script>
@endpush
