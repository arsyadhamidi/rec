...
class DirekturCutiTahunanController extends Controller
{
    public function index(Request $request)
    {
        ...
        return view('direktur.kehadiran.cuti-tahunan.index');
    }
    ...
    public function generatepdf(Request $request)
    {
        ...
        $pdf = PDF::loadview('direktur.kehadiran.cuti-tahunan.export-pdf', ['cutis' => $cutis])->setPaper('A4', 'potrait');
    ...
    public function diterima($id)
    {
        ...
        $kuotaCutis = KuotaCutiTahunan::firstOrCreate(
            [
                'user_id' => $cutis->user_id,
                'tahun' => $cutis->tahun
            ],
            [
                'kuota_awal' => 12,
        ...
        $tglMulai = Carbon::parse($cutis->tgl_mulai);
        $tglSelesai = Carbon::parse($cutis->tgl_selesai);
        $jumlahCuti = $tglSelesai->diffInDays($tglMulai) + 1;

        // Hitung sisa kuota cuti
        $sisaKuotaCutis = $kuotaCutis->kuota_awal - $kuotaCutis->kuota_terpakai;

        if ($sisaKuotaCutis < $jumlahCuti) {
            return back()->with('error', 'Maaf! Sisa Kuota Cuti Anda tidak mencukupi, silakan lapor ke pihak SDM.');
        }
        ...
        // Update kuota cuti terpakai
        $kuotaCutis->increment('kuota_terpakai', $jumlahCuti);
